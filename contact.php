...
if($form_submitted == true && $errors_detected == false){
	...
	if(isset($clean['name'])){
		$value_name = htmlentities($clean['name']);
	}else{
		$value_name = '';
	}
	if(isset($clean['phone'])){
		$value_phone = htmlentities($clean['phone']);
	}else{
		$value_phone = '';
	}
	if(isset($clean['email'])){
		$value_email = htmlentities($clean['email']);
	}else{
		$value_email = '';
	}
	if(isset($clean['message'])){
		$value_message = htmlentities($clean['message']);
	}else{
		$value_message = '';
	}
	
	$file_form = 'templates/form.html';
	$tpl_form = file_get_contents($file_form);
	$place_holders = array(
							'[+legend+]',
							'[+p1+]',
							'[+p2+]',
